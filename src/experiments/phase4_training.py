"""Phase 4 copy of frozen trainer with observational stability checks only."""
import json
import random
import time
...
from src.experiments.stability import StabilityMonitor
...
def train(config: dict, smoke: bool = False, run_dir: Path | None = None,
    ...
    monitor = StabilityMonitor(model, run, state.get('stability') if resume and (run/'last.pt').exists() else None)
    ...
    for epoch in range(start_epoch,t['epochs']):
        ...
        for batch_index,(images,targets) in enumerate(training):
            images,targets=images.to(device),targets.to(device)
            optimizer.zero_grad(set_to_none=True)
            monitor.begin(epoch,batch_index,lr,images,targets)
            logits=model(images)
        ...
        state={'history':history,'elapsed_seconds':elapsed,'best_epoch':best_epoch,'stability':monitor.state}
        monitor.save()
